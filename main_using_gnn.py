...
def train_model(model, train_loader, val_loader, optimizer, criterion, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for home_data, away_data, context, target in tqdm(train_loader, desc="Training", leave=False):
        ...
        with torch.no_grad():
            for home_data, away_data, context, target in tqdm(val_loader, desc="Validation", leave=False):
                home_data = home_data.to(device)
                away_data = away_data.to(device)
                context = context.to(device)
                target = target.to(device)
